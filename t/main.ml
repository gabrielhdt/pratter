...
let token = Fun.id
let expr ~ops = Pratter.(expression ~appl ~token ~ops |> run)
...
module RTTerm :
  ...
  let equal t u =
    ...
    | Error (`Op_conflict t), Error (`Op_conflict u) -> Terms.equal t u
    | _, _ -> false
  ...
  let pp oc t =
    ...
    | Error (`Op_conflict t) ->
        Format.fprintf oc "@[Operator conflict on@ \"%a\"@]" Terms.pp t
end
...
let simple_nothing () =
  ...
  let f = symb "f" in
  let x = symb "x" in
  let not_parsed = List.to_seq [ f; x ] in
  let parsed = return @@ add_args f [ x ] in
  Alcotest.check rtterm "f x" (expr ~ops not_parsed) parsed

let simple_infix () =
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Left, 1.0, s) ]
    | _ -> []
  in
  let x = symb "x" in
  let y = symb "y" in
  let not_parsed = List.to_seq [ x; symb "+"; y ] in
...
let two_operators () =
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Left, 1.0, s) ]
    | Terms.Symb "*" as s -> Pratter.[ (Infix Left, 1.1, s) ]
    | _ -> []
  in
  let x = symb "x" in
  let y = symb "y" in
  let z = symb "z" in
  let not_parsed = List.to_seq [ x; symb "+"; y; symb "*"; z ] in
  let parsed =
    return
    @@
...
let appl_opertor () =
  ...
  let f = symb "f" in
  let x = symb "x" in
  let not_parsed = List.to_seq [ f; x; symb "+"; x ] in
  let parsed = return @@ add_args (symb "+") [ appl f x; x ] in
  Alcotest.check rtterm "f x + x" (expr ~ops not_parsed) parsed

let ops = function
  | Terms.Symb "!" as s -> [ (Pratter.Prefix, 1.0, s) ]
  | _ -> []

let simple_prefix () =
  let x = symb "x" in
  let not_parsed = List.to_seq [ symb "!"; x ] in
...
let prefix_appl () =
  let x = symb "x" in
  let f = symb "f" in
  let not_parsed = List.to_seq [ symb "!"; f; x ] in
...
let prefix_appl_in () =
  ...
  let f = symb "f" in
  let fac = symb "!" in
  let not_parsed = List.to_seq [ f; fac; x ] in
  let parsed =
    return
    @@
...
let double_prefix () =
  (* !!x = !(!x) *)
  let not_parsed = List.to_seq [ symb "!"; symb "!"; symb "x" ] in
...
let precedences_left_same () =
  ...
  in
  let not_parsed =
    List.to_seq [ symb "x"; symb "+"; symb "y"; symb "*"; symb "z" ]
  in
  let parsed =
    return
    @@
...
let precedences_right_same () =
  (* x + y * z = x + (y * z) when bp(+) = bp( * ) and both are right
     associative *)
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Right, 1.0, s) ]
    | Terms.Symb "*" as s -> Pratter.[ (Infix Right, 1.0, s) ]
    | _ -> []
  in
  let not_parsed =
...
let precedences_lt_not_assoc () =
  (* x + y * z = x + (y * z) when bp(+) < bp( * ) and both are not
     associative *)
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Neither, 0., s) ]
    | Terms.Symb "*" as s -> Pratter.[ (Infix Neither, 0.1, s) ]
    | _ -> []
  in
  let not_parsed =
    List.to_seq [ symb "x"; symb "+"; symb "y"; symb "*"; symb "z" ]
...
let precedences_gt_not_assoc () =
  ...
  let ops = function
    ...
    | Terms.Symb "+" as s -> Pratter.[ (Infix Neither, -1., s) ]
    | Terms.Symb "*" as s -> Pratter.[ (Infix Neither, -1.1, s) ]
    | _ -> []
  in
  let not_parsed =
    List.to_seq [ symb "x"; symb "+"; symb "y"; symb "*"; symb "z" ]
...
let mixing_una_bin () =
  (* -x + y = (-x) + y when bp(-) > bp(+) *)
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Neither, 1.0, s) ]
    | Terms.Symb "-" as s -> Pratter.[ (Prefix, 1.1, s) ]
    | _ -> []
  in
  let not_parsed = List.to_seq [ symb "-"; symb "x"; symb "+"; symb "y" ] in
  let parsed =
...
let mixing_una_bin_bis () =
  ...
  let ops = function
    | Terms.Symb "+" as s -> Pratter.[ (Infix Neither, 1.0, s) ]
    | Terms.Symb "!" as s -> Pratter.[ (Prefix, 0.9, s) ]
    | _ -> []
  in
  let not_parsed = List.to_seq [ symb "!"; symb "x"; symb "+"; symb "y" ] in
  let parsed =
...
let postfix_mixes_infix () =
  let ops = function
    | Terms.Symb "!" as s -> [ (Pratter.Postfix, 1.0, s) ]
    | Terms.Symb "+" as s -> Pratter.[ (Infix Left, 0.9, s) ]
    | Terms.Symb "/" as s -> Pratter.[ (Infix Left, 1.1, s) ]
    | _ -> []
  in
  let raw = List.to_seq [ symb "x"; symb "!"; symb "+"; symb "y" ] in
  let parsed =
    return @@ add_args (symb "+") [ add_args (symb "!") [ symb "x" ]; symb "y" ]
  in
  Alcotest.check rtterm "x ! + y = @(+, @(!, x), y)" (expr ~ops raw) parsed;
  let raw = List.to_seq [ symb "x"; symb "+"; symb "y"; symb "!" ] in
  let parsed =
...
let postfix_mixes_prefix () =
  let ops = function
    ...
    | _ -> []
  in
  let raw = List.to_seq [ symb "-"; symb "x"; symb "!" ] in
  let parsed =
    return @@ add_args (symb "!") [ add_args (symb "-") [ symb "x" ] ]
  in
...
let both_infix_prefix () =
  let ops = function
    | Terms.Symb "-" as s -> Pratter.[ (Prefix, 2.0, s); (Infix Left, 1.0, s) ]
    | _ -> []
  in
  let raw = List.to_seq [ symb "-"; symb "x"; symb "-"; symb "y" ] in
  let parsed =
    return @@ add_args (symb "-") [ add_args (symb "-") [ symb "x" ]; symb "y" ]
  in
...
let ops = function
  | Terms.Symb "+" as s -> Pratter.[ (Infix Left, 1.1, s) ]
  | Terms.Symb "*" as s -> Pratter.[ (Infix Right, 1.5, s) ]
  | Terms.Symb "=" as s -> Pratter.[ (Infix Neither, 0.5, s) ]
  | Terms.Symb "!" as s -> [ (Pratter.Postfix, 1.0, s) ]
  | Terms.Symb "-" as s -> [ (Pratter.Prefix, 0.9, s) ]
  | _ -> []
...
let constant_leaf_amount =
  ...
  QCheck.(
    Test.make ~name:"parsing_constant_leaf_amount" ~count:100 term_list
      (fun l ->
        assume (l <> []);
        match expr ~ops (List.to_seq l) with
        | Ok t ->
            Terms.count_symb t
...
let empty_table_id =
  QCheck.Test.make ~name:"parsing_identity_empty_table" ~count:100 term_list
    (fun l ->
      ...
      in
      let parsed = expr ~ops:(fun _ -> []) (List.to_seq l) in
...
let compare_yacc =
  QCheck.(
    Test.make ~name:"compare_yacc" ~count:1000 slist (fun l ->
        let res = expr ~ops (List.to_seq l) in
...
let _ =
  ...
  run "Simple terms"
    [
      ("nothing", [ test_case "nothing" `Quick simple_nothing ])
    ; ( "infix"
      , [
